        ...
        success: function(data){
        	if(data.status == 0){
        		...
        		$.each(memList,function(i,mem){
        			var htmls=""; 
        			htmls+='<tr><td><img class="headicon" src="'+mem.picurl+'">'
					...
					htmls+='<td><a class="editbtn" onclick="editMember(this)" detail=\''+JSON.stringify(mem)+'\'></a>'
						+'<a class="delbtn" '
						+'onclick="alertConfirm(\'2\',\'确定删除该成员吗？\',\'delMem()\',this)"></a>'
...
function saveTeam(){
	...
	var mid=sessionStorage.getItem("currentmid");
	if(tname.trim()==""){
		alertMsg("2","请输入队伍名称!","fail")
		return;
	}else if(tname.indexOf(",")!=-1){
		alertMsg("2","队伍名称不允许出现非法字符！","fail");
		return;
	}
	if($(".teaList tbody tr").length==0){
		alertMsg("2","请添加指导教师！","fail")
		return;
	}
	if($(".stuList tbody tr").length==0){
		alertMsg("2","请添加队员！","fail")
		return;
	}
	if(orgtype.trim()==""){
