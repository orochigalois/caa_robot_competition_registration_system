$(function(){
	...
	bmsgetcurmatchname();
	var jsonobj=JSON.parse(sessionStorage.getItem("TClistJson"));
	var num="";
	...
	$("#page").pagination({
		...
		remote : {
			...
			pageParams : function(data) {
				...
				param.sex=sex;
				param.str=str;
				sessionStorage.setItem("TClistJson", JSON.stringify(param));
				return {
				...
				if (data.status == 0) {
					...
					$.each(data.list,function(i,mem){
						htmls+='<tr><td><img class="memberhead" src="'+mem.picurl+'">'
				    			+'<span>'+mem.uname+'</span></td><td>TC</td>'
						...
						if(sessionStorage.getItem("roleflg")=="00"||sessionStorage.getItem("roleflg")=="04"){
							
							htmls+='<td>'
								+'<a uid="'+mem.uid+'" class="delete1" '
								+'onclick="alertConfirm(\'2\',\'确定删除该TC吗？\',\'delTC()\',this)">删除</a>'
...
function addTC(){
	...
	htmls+='<div class="cover2"></div><div id="addAdminbox">'
   			+'<img class="closeWin" src="../images/guanbi.png" onclick="closeInfoWin()">'
   			+'<h1 class="wTitle">添加TC</h1><div class="infobox">'
   			+'<ul class="infoboxul"><li><span class="leftspan">姓名</span>'
