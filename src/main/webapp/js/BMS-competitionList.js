$(function(){
	//getEvent();
	bmsgetcurmatchname();
	var jsonobj=JSON.parse(sessionStorage.getItem("complistJson"));
	var num="";
	...
	$("#page").pagination({
		...
		remote : {
			...
			pageParams : function(data) {
				...
				sessionStorage.setItem("complistJson", JSON.stringify(param));
				return {
...
function showTCList(obj){
	...
	htmls+='<div class="shade"></div><div class="TCwrap"><h1 class="wTitle">TC管理</h1>'
    	 ...
    	 +'</div><span class="neckright" onclick="getRestTc(\''+rid+'\')">分配TC</span>'
    	 +'</div>';
	$.ajax({
		type: "GET",
        ...
        success: function(data){
        	if(data.status == 0){
        		...
        		if(data.list.length==0){
        			htmls+='<h1 class="wTitle" style="margin-top:20px;font-weight:normal">该赛项暂未分配TC！</h1>'
        		}else{
	...
	$.ajax({
        ...
        success: function(data){
        	if(data.status == 0){
        		console.log(data);
        		if(data.list.length==0){
        			htmls+='<h1 class="wTitle" style="margin-top:20px;font-weight:normal">暂无可分配的TC,请添加TC！</h1>'
        			htmls+='<div class="btnwrap">'
           				 +'<span style="color:#333;border:1px solid #ccc;" onclick="closeTCList()">知道了</span></div></div>';
        		}else{
