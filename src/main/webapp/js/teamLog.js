...
function getAllRCJTeam(num){
	...
	$("#page").pagination({
	    ...
	    jumpBtnText:'跳转',
		remote : {
			"url" : "../findTeamsToRCJBySignuidPages",
			pageParams : function(data) {
				var param={};
				param.pageNo=data.pageIndex;
				sessionStorage.setItem("userteamlogJson", JSON.stringify(param));
				return {
				...
				if (data.status == 0) {
					...
					$.each(data.list,function(i,team){
						...
						if(compareTime()){
							...
							+'<a class="manageteam2" onclick="subRcjLog(this)">提交日志</a>'
							+'</td></tr>'
						}
					})
					$("tbody").append(htmls)
...
function subRcjLog(obj){
	var tid=$(obj).parent().attr("tid");
	var rid=$(obj).parent().attr("rid");
	sessionStorage.setItem("tid",tid);
	sessionStorage.setItem("rid",rid);
	
	window.open("teamSubLog.html","_self");
}
